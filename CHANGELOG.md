...
- [修复] Canvas 生成图片跨设备可恢复：Canvas 账号生成成功后节点/工作台记录现在保存 BFF 云端 asset 引用（`assetId`，图片本体已存入 MinIO 媒体存储）；再次打开画布或换设备时若本地缓存缺失，自动通过 BFF 签名 URL 从云端媒体存储拉回并重建本地缓存，不再显示死链 `blob:` 空图。图片工作台的下载/加入参考/保存资产同样支持云端回源。
