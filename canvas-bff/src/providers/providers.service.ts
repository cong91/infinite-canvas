...
export const providerKeyBody = z.object({ name: z.string().trim().min(1).max(120), groupId: z.string().trim().min(1).max(200) });
...
export class ProvidersService {
    ...
    async create(accountId: string, sessionToken: string | undefined, input: z.output<typeof providerBody>) {
        ...
        if (input.catalogKeyId && !secret) { const resolved = await this.catalog.getApiKeySecret(await this.upstreamToken(sessionToken), input.catalogKeyId); secret = resolved.secret; catalogItem = resolved.item; }
