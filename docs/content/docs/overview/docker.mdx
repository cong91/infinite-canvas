...
This provisions PostgreSQL 16, runs the checked-in BFF migrations, creates the `canvas-media` bucket in MinIO, and starts the BFF on `http://localhost:17372`. The current BFF still uses in-memory repositories, so this verifies container wiring, migration bootstrap and health only; it does not claim durable Canvas persistence yet. Stop it with `docker compose -f docker-compose.bff.local.yml -p infinite-canvas-bff down`; add `-v` only when intentionally deleting test data.
