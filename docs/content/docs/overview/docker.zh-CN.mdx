...
该 stack 会启动 PostgreSQL 16、执行仓库中的 BFF 迁移、在 MinIO 创建 `canvas-media` bucket，并在 `http://localhost:17372` 启动 BFF。当前 BFF 仍使用内存仓储，因此这里只验证容器连线、迁移初始化和健康检查，不代表 Canvas 数据已经持久化。停止命令为 `docker compose -f docker-compose.bff.local.yml -p infinite-canvas-bff down`；只有确认要删除测试数据时才添加 `-v`。
