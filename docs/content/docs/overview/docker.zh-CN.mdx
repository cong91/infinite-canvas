...
仓库提供面向 `ovh-sing` 的生产 Compose stack 与 SSH 部署 workflow。前端、BFF、PostgreSQL 和 MinIO 端口都只绑定回环地址；主机 Nginx 只暴露 `https://canvas.v-claw.org`，将 `/api/` 转发到 BFF，并通过 `/storage/` 提供 MinIO 签名媒体地址。

先在服务器创建 `/srv/infinite-canvas/.env.production`，可从 `deploy/ovh-sing/env.production.example` 复制，权限设为 `600`，并在 Sub2API 使用相同的 `SUB2API_CANVAS_BFF_SECRET`。首次部署要求 `canvas.v-claw.org` 已有可解析的 DNS 记录，脚本会通过 Certbot 申请证书。要在推送到 `main` 时自动部署，需要配置 GitHub Actions secrets：`OVH_SSH_HOST`、`OVH_SSH_USER`、`OVH_SSH_PRIVATE_KEY` 和 `OVH_SSH_KNOWN_HOSTS`。
