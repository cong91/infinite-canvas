...
- The Canvas BFF now provides the account/session/provider/project/asset/generation contract and Sub2API SSO handoff; the Canvas library and project page hydrate, create, rename, delete and autosave projects through the account session, while asset hydration and generation adapter wiring are still pending before the local workbench path can be replaced.
- The BFF uses PostgreSQL repositories and S3-compatible object storage in non-test environments; in-memory adapters are retained only for contract tests. Provider HTTP adapters and browser-side remote asset hydration remain follow-up work.
- The existing local workbench continues to keep its legacy API keys and browser data locally until the remote generation path is enabled for that surface. Do not treat localforage or WebDAV as the account source of truth.
